...
public class PlayerDemo extends Player {
    ...
    //Creates the random target to be attacked
    //Starts as -1 so the game can load in the operatives
    public static int randOper = -1;
    ...
    private int nodeNum;
    /**
     * Select an new operative to attack
     */
    public void chooseTarget() {
        //randOper is -1 on launch as the operatives haven't yet been created.
        //The first random target is generated by the GameScreen and sent here.
        if(randOper != -1) {
            //Make sure their are still operatives to attack
            int i = 0;
            target = gScreen.remainingOperatives.get(randOper);
            while (target.dead) {
                i += 1;
                if (i >= gScreen.remainingOperatives.size()) {
                    ...
                    //Generate the next random target
                    Random rand = new Random();
                    randOper = rand.nextInt(gScreen.remainingOperatives.size());
                    target = gScreen.remainingOperatives.get(randOper);
                }
            }
